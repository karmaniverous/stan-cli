...
1.  **Execute:** Run `stan run`.
    - This executes _all_ configured validation scripts (`test`, `lint`, `build`, `knip`, `docs`, `smoke`, etc.).
...
- _Green & Complete?_ → **Release**.
- _Red or New Requirements?_ → **Snap**.
...
1.  **Baseline:** Run `stan snap`.
    - This updates the internal snapshot so `archive.diff.tar` will contain only changes made _after_ this point.
...
3.  **Paste:** Start a new chat, paste the handoff, and attach the _full_ `archive.tar`.
